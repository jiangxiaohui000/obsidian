## 函数柯里化
```js
// 柯里化的核心是把多参数函数拆成多次单参数调用。  
// 实现上通常通过 `fn.length` 判断参数是否收集完成，  
// 利用闭包缓存参数，参数齐全后再一次性执行原函数，  
// 同时要注意 this 的透传问题。
...
function curry(fn) {
	return curried(...args) {
		...
		if (args.length >= fn.length) {
			return fn.apply(this, args); // 保证 `this` 不丢失
		}
...
function _new(target, ...args) {
    ...
    // 绑定 this 并执行构造函数，将 obj 绑定为构造函数内部的 this，
...
// 以下两种方法都行
async function asyncPool(urls, limit) {
	let pool = new Set();
	let index = 0;
	let result = [];
	return new Promise(resolve => {
		let addTask = () => {
			if (index >= urls.length && pool.size === 0) {
				resolve(result);
				return;
			}
			while (index < urls.length && pool.size < limit) { // 启动阶段填满并发池
...
async function asyncPool(tasks, limit) { // tasks是一个promise数组
	let index = 0;
	let running = 0;
	let result = [];
	return new Promise(resolve => {
...
class A {
	...
	c = () => {
		console.log(this.name); // 箭头函数的this永远指向实例对象
	}
}
...
const d = a.b;
const e = a.c;
d(); // TypeError: Cannot read properties of undefined (reading 'name')  非严格模式this===window 严格模式this===undefined
e(); // jj this===a
```
...
//	{id:11, parent: 1 , name:'11'}, 
//	{id:111 , parent: 11 , name:'111'}, 
//	{id:2 , parent: 0 , name:'2'}, 
//	{id:22 , parent: 2, name:'22'}, 
// ]
