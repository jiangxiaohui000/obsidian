...
fn(); // undefined
// JavaScript 的运行过程拆分为两个阶段：**编译阶段**（预解析）和 **执行阶段**。
// 变量提升是发生在代码编译阶段，在执行之前。所以编译完是这样的
function fn(){
    var tmp; // 声明被提升
...
// 如果tmp是用let或者const声明，存在暂时性死区，不会提升，函数执行在全局作用域中，就能打印出外层tmp了。
```
## 优先级
...
// 编译后
// --- 提升阶段 ---
function fn(){} // 函数声明被提升到顶部，此时 fn 是函数
// var fn;      // 变量声明被提升，但因为同名函数已存在，这步声明被忽略
// --- 执行阶段 ---
console.log(fn); // 1. 此时 fn 依然是提升阶段确定的函数-> 打印: [Function: fn]    
fn = 2019;       // 2. 执行赋值操作，fn 的值从函数变成了数字 2019
console.log(fn); // 3. 此时 fn 是 2019-> 打印: 2019
// function fn(){} // 4. 这行声明已经在提升阶段处理过了，执行阶段跳过
```
